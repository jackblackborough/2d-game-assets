        ...
        for map_type, tex_map_list in tex_map_dict.items():
            ...
            for suffix_preferred in suffix_list:
                for tex_map in tex_map_list:
                    _, suffix = os.path.splitext(tex_map.filename)
                    if suffix.lower() == suffix_preferred:
                        tex_maps.append(tex_map)
                        found = True
            if not found:
...
class Hdri(BaseTex):
    ...
    def get_size_list(self,
        ...
        map_preferences: User map preferences, used for TEXTURES and if local_only;
        """
        size_list_light = self.light.get_size_list(
            incl_watermarked, local_only, addon_convention, local_convention)
        size_list_bg = self.bg.get_size_list(
            incl_watermarked, local_only, addon_convention, local_convention)

        if not (len(size_list_bg) and len(size_list_bg)):
            return []

        size_list = size_list_light
    ...
    def get_maps(self,
        ...
        light_maps = self.light.get_maps(
            workflow,
            size,
            lod,
            prefer_16_bit,
        ...
        # Return a full list only if BOTH bg and light are found
        if not (len(bg_maps) and len(light_maps)):
            return []

        bg_maps.extend(light_maps)
        return bg_maps
    ...
    def get_mesh(self,
        ...
        for mesh in self.meshes:
            if model_type is not None and mesh.model_type != model_type:
                continue
            # Non-FBX should not be concerned with LOD specifics
            if model_type == ModelType.FBX and mesh.lod != lod:
                continue
            meshes.append(mesh)
        return meshes
...
class AssetData:
    ...
    def get_days_since_published(self,
                                 as_utc_timezone: bool = False
                                 ) -> Optional[int]:
        if not self.published_at:
            return None
        if as_utc_timezone:
            # For comparing values (e.g. to define if it is a new asset) always
            # use the utc timezone (as_utc_timezone = True);
            published_datetime = datetime.fromtimestamp(self.published_at,
                                                        tz=timezone.utc)
            now = datetime.now(timezone.utc)
        else:
            published_datetime = datetime.fromtimestamp(self.published_at)
            now = datetime.now()
        return max((now - published_datetime).days, 0)
    ...
    def is_new(self) -> bool:
        ...
        days_old = self.get_days_since_published(as_utc_timezone=True)
        if days_old is None:
            return False
        days_threshold = 30
