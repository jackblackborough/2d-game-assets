...
class PoliigonConnector():
    ...
    def _flatten_results(self, resp: Dict) -> Dict:
        ...
        if type(results) is not dict:
            ...
            #                the same structure.
            self.logger.debug("Non-dict results from API")
            return resp
        ...
        for _key in resp:
            ...
            if _key in results:
                ...
                self.logger.warning(f"Duplicate key: {_key}")
                _key += "_2"
    ...
    def check_login_with_website_success(
        ...
        if not res.ok:
            if res.error != ERR_NOT_AUTHORIZED:
                ...
                self.logger.error(f"Validation error {res.error}")
        elif not api_proposed_ok and not api_current_ok:
            # TODO(SOFT-603): Error handling
            pass
        else:
...
#        return res
    ...
    def set_recently_viewed_assets(self, asset_ids: List[int]) -> ApiResponse:
        endpoint = "/recently/viewed"
        payload = {"language_code": "en",  # TODO(Joao): Set Language
                   "asset_ids": asset_ids}
    ...
    def get_recently_viewed_assets(self) -> ApiResponse:
        endpoint = "/users/recently/viewed/assets"
        res = self._request_authenticated(endpoint,
                                          payload=None,
                                          api_v2=True,
                                          method="GET")
        ...
        if not res.ok:
            if res.error not in SKIP_REPORT_ERRS:
                self.report_message(
                    "get_recently_viewed_error", f"{res.error}", "error")
        return res
    ...
    def signal_view_screen(self, screen_name: str) -> ApiResponse:
        ...
        if screen_name not in valid_screens:
            self.logger.error(f"Screen name is not valid: {screen_name}")
            return ApiResponse(
