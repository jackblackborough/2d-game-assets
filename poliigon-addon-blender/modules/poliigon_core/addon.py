...
class PoliigonAddon():
    ...
    recent_viewed_fetched: bool = False
    ...
    # Timestamp of the last page loaded (last iteration) with the addon
    addon_last_view: Optional[datetime] = None
    ...
    def __init__(self,
        ...
        self.addon_last_view = self.load_addon_last_view_time()
    ...
    def set_api_token(self) -> None:
        ...
        # We're falling back to "" if addon_root_path is not yet set, just an
        # extra layer of safety - addon_root_path should not be None at this point;
        base_path = self.addon_root_path if self.addon_root_path is not None else ""
        access_token_file = os.path.join(base_path, "access_token.txt")
        if local_token:
            self._api.token = local_token
        else:
            self.inject_token(access_token_file)

        # If a token file is found, even if not used, it should be removed
    ...
    def set_addon_last_view_time(self):
        """Saves the timestamp of the latest browsing iteration
        of the user with the addon."""
        self.addon_last_view = datetime.now(timezone.utc)
        self.settings_config.set("user",
                                 "addon_last_view",
                                 self.addon_last_view.strftime("%Y-%m-%d %H:%M:%S"))
        self._settings.save_settings()

    def load_addon_last_view_time(self) -> Optional[datetime]:
        last_view = self.settings_config.get("user",
                                             "addon_last_view",
                                             fallback=None)
        try:
            last_view = datetime.strptime(
                last_view, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
    ...
    def refresh_top_level_queries_flags(self):
        ...
        self.recent_viewed_fetched = False

    def are_user_assets_fetched(self) -> bool:
        user_assets_fetched = self.recent_downloads_fetched and self.recent_viewed_fetched
        if self.user_legacy_own_assets():
            return self.my_assets_fetched and user_assets_fetched
        return user_assets_fetched
