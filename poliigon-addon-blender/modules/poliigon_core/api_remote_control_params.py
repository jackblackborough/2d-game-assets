...
KEY_TAB_RECENTLY_VIEWED = "recently_viewed"
...
class ApiJobParamsLogin(ApiJobParams):
    ...
    def finish(self, api_rc, job) -> None:
        ...
        if not job.result.ok or self.mode == CmdLoginMode.LOGOUT:
            ...
            api_rc._addon.refresh_top_level_queries_flags()
...
class ApiJobParamsGetAssets(ApiJobParams):
    ...
    @staticmethod
    def _get_ids_from_asset_dict_list(asset_dict_list: List[Dict]) -> List[int]:
        """Gets the ids from the recently viewed nested response structure"""
        asset_id_list = []
        if asset_dict_list is None:
            return asset_id_list
        for asset_dict in asset_dict_list:
            asset_data_dict = asset_dict.get("asset_data")
            if asset_data_dict is None:
                continue
            asset_id = asset_data_dict.get("AssetID")
            if asset_id is not None:
                asset_id_list.append(asset_id)
        return asset_id_list

    def _get_asset_ids_from_body(self, job) -> List[int]:
        """Returns list of asset IDs for all involved request types
        ...
        elif self.tab == KEY_TAB_RECENTLY_VIEWED and "results" in job.result.body:
            asset_data_list = job.result.body.get("results")
            asset_id_list = self._get_ids_from_asset_dict_list(asset_data_list)
        return asset_id_list

    def _unexpected_endpoint_result(self, api_rc, is_toplevel: bool = False):
        error_id = "get_assets_unexpected_case"
        if is_toplevel:
            error_id = "get_toplevel_assets_unexpected_case"

        msg = f"Unexpected case\n    {str(self)}"
        api_rc.logger.error(msg)
        api_rc._api.report_message(error_id, msg, "error")
    ...
    def thread_execute(self, api_rc, job) -> None:
        ...
        if self.is_my_assets_top_level():
            ...
            api_rc.logger.debug("Skipping my_assets_fetched change")
        elif self.is_recent_downloads_top_level():
            # api_rc._addon.recent_downloads_fetched = False
            api_rc.logger.debug("Skipping my_assets_fetched change")
        elif self.is_recent_viewed_top_level():
            api_rc._addon.recent_viewed_fetched = False
        ...
        # TODO(Joao): Remove the use of page_size on api rc
        ...
        if is_toplevel:
            if self.tab == KEY_TAB_MY_ASSETS:
                ...
                job.result = ApiResponse(ok=True,
                                         ...
                                         error="")
            elif self.tab == KEY_TAB_RECENTLY_VIEWED:
                job.result = api_rc._api.get_recently_viewed_assets()
            else:
                # During normal operation this case should not happen.
                # Basically we are using the "get all" job to populate
                # AssetIndex and from then on, the above three cases are
                # all that's needed for toplevel requests.
                job.result = self._unexpected_endpoint_result(api_rc, is_toplevel)
        else:
            if self.category_list != [CATEGORY_ALL]:
                ...
                job.result = self._unexpected_endpoint_result(api_rc)
    ...
    def _set_user_last_view(self, api_rc, job):
        if api_rc._addon.user is None:
            return
        user_last_view = None
        asset_data_list = job.result.body.get("results", [])
        for asset_dict in asset_data_list:
            try:
                asset_last_view = asset_dict.get("last_viewed")
                asset_last_view_date = datetime.strptime(asset_last_view,
                                                         "%Y-%m-%dT%H:%M:%S.%fZ"
                                                         ).replace(tzinfo=timezone.utc)
            except (ValueError, TypeError):
                continue
            if user_last_view is None or asset_last_view_date > user_last_view:
                user_last_view = asset_last_view_date
        api_rc._addon.user.last_poliigon_view = user_last_view
        api_rc._addon.user.count_assets_recent_viewed = len(asset_data_list)
    ...
    def _finish_toplevel_requests(self, api_rc, job) -> List[int]:
        ...
        elif self.tab == KEY_TAB_LOCAL or self.tab == KEY_TAB_RECENTLY_VIEWED:
            asset_ids_resp = self._get_asset_ids_from_body(job)
            api_rc._asset_index.store_query(
                asset_ids_resp,
                key,
        ...
        if "total" not in job.result.body.keys():
            job.result.body["total"] = len(asset_ids_resp)

        # Set the last viewed asset date as user.last_poliigon_view;
        # This should at some point be moved to /me endpoint;
        if self.tab == KEY_TAB_RECENTLY_VIEWED:
            self._set_user_last_view(api_rc, job)
    ...
    def _filter_for_tabs(self,
                         ...
                         is_later_page: bool):
        asset_ids_filtered = []
        if self.tab in [KEY_TAB_MY_ASSETS, KEY_TAB_RECENT_DOWNLOADS, KEY_TAB_RECENTLY_VIEWED]:
    ...
    def is_recent_viewed_top_level(self) -> bool:
        return self._is_toplevel_request() and self.tab == KEY_TAB_RECENTLY_VIEWED

    def finish(self, api_rc, job) -> None:
        """Finishes get asset jobs, called from thread_collect."""

        if api_rc.in_shutdown or not job.result.ok:
            return
        ...
        if self.is_my_assets_top_level():
            ...
            api_rc._addon.recent_viewed_fetched = True

        if not self._is_toplevel_request():
            # If not top level requests, consider user navigation on the
            # browser and save the last_view in addon settings;
            with api_rc.lock_last_viewed:
                api_rc._addon.set_addon_last_view_time()

        if self.already_in_index:
            return
