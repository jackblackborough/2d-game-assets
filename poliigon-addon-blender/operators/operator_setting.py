...
from ..toolbox import get_context, ModeUpdate
...
class POLIIGON_OT_setting(Operator):
    ...
    tooltip: bpy.props.StringProperty(default="", options={"HIDDEN"})  # noqa: F722, F821  # type: ignore
    mode: bpy.props.StringProperty(default="", options={"HIDDEN"})  # noqa: F722, F821  # type: ignore
    ...
    def _set_area(self,
        ...
        update_assets = "asset_filter" in self.mode

        # If we are removing the filters, set persist_filters back to default
        if closing:
            props = bpy.context.window_manager.poliigon_props
            if props:
                props.persist_filters = False

        return cTB.set_area(area, update_assets)
    ...
    def _clear_search(self) -> None:
        ...
        elif self.mode.endswith(KEY_TAB_RECENTLY_VIEWED):
            self._clear_search_tab(tab=KEY_TAB_RECENTLY_VIEWED)
    ...
    def execute(self, context):
        ...
        if self.mode in ["none", ""]:
            return {"FINISHED"}
        elif self.mode == "close_filters":
            do_update = self._set_area(area=KEY_TAB_ONLINE, closing=True)
            ...
            do_update = self._set_area()
        elif self.mode == "check_default_filter":
            do_update = self._set_default_filter()
        else:
