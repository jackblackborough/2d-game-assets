...
def texture_name(filename_tex, size, file_format):
    """ Convention 1 tex maps have no size in filename,
    but we need unique image names for consecutive imports with different sizes to work correctly
    """
    name_tex = os.path.splitext(filename_tex)[0]
    if size not in name_tex:
        name_tex += f"_{size}"
...
class CyclesMaterial():
    ...
    def configure_tex_node_image(
        ...
        name_tex = texture_name(filename_tex, tex_map.size, file_format)
    ...
    def reuse_simple_uv_group(
        ...
        """
        node_group = node_utils.reuse_uv_group_node(
            parent_frame=parent_frame,
            name=NAME_GROUP_SIMPLE_UV,
            mat=self.mat,
            scale=self.params.scale,
            width=W_NODE_WIDE,
            rotation=self.params.global_rotation,
    ...
    def create_simple_uv_group(
        ...
        node_group = node_utils.prepare_simple_uv_group_node(
            parent_frame=parent_frame,
            mat=self.mat,
            name=NAME_GROUP_SIMPLE_UV,
            scale=self.params.scale,
