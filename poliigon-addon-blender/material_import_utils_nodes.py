...
def reuse_uv_group_node(
        parent_frame: bpy.types.Node,
        name: str,
    ...
    """ Reuses an already created node tree to create a new Simple UV Mapping node group. """

    if name not in bpy.data.node_groups.keys():
        return None

    node_group = create_group_node(
        group=mat,
        parent=parent_frame,
...
def uv_group_node_set_defaults(
        node_group: bpy.types.Node,
        scale: float,
        rotation: float = 0,
        translate_x: float = 0,
        translate_y: float = 0,
        aspect_ratio: float = 1):
    """ Set all values inside a UV Mapping node """
    set_value(
        node=node_group,
        sock_name="Scale",
        sock_bl_idname_expected="NodeSocketFloat",
...
def prepare_simple_uv_group_node(
        ...
        aspect_ratio: float = None) -> bpy.types.ShaderNodeGroup:
    """ Create a NEW UV Mapping Node that doesnt already exist """
    # Create node
    node_group = create_group_node(
        group=mat,
        parent=parent_frame,
    ...
    # Set default values
    uv_group_node_set_defaults(node_group,
                               scale=scale,
                               rotation=rotation,
                               translate_x=translate_x,
                               translate_y=translate_y,
    ...
    if location:
        node_group.location = location

    return node_group


def create_uv_group_node(
        parent_frame: Optional[bpy.types.Node],
        mat: bpy.types.Material,
        name: str,
        scale: float,
        location: mathutils.Vector = None) -> bpy.types.Node:
    """Creates Poliigon's Simple UV Mapping node group."""

    node_group = reuse_uv_group_node(parent_frame, name, mat, scale)
    if node_group is not None:
        # update some values before returning
        node_group.location = location
        set_value(
            node=node_group,
            sock_name="Scale",
    ...
    node_group = prepare_simple_uv_group_node(parent_frame, mat, name, scale, location)
