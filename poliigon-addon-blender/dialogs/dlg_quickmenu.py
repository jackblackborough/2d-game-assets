...
from ..panels.panel_hdri import get_hdri_size_common
    ...
    def draw_hdri_sizes(
        ...
        if size in downloaded:
            ...
            if cTB.settings["hdri_use_jpg_bg"]:
                ...
                # Find if EXR or HDR
                size_options = get_hdri_size_common(asset_id, is_bg=False)
                # size_options = get_size_options_common(asset_id, is_bg=False)
                for sublist in size_options:
                    if f"{size}_EXR" in sublist:
                        op.size_bg = f"{size}_EXR"
                        break
                    elif f"{size}_HDR" in sublist:
                        op.size_bg = f"{size}_HDR"
                        break
            op.tooltip = tip
