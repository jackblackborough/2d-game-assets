...
def _build_assets_no_assets(cTB, area: str, category: str) -> None:
    ...
    elif area == KEY_TAB_RECENTLY_VIEWED:
        label = _t("No Recently Viewed {0} found").format(category)
...
def _draw_button_quick_preview(
    ...
    if cTB.is_unlimited_user():
        op.tooltip = _t("Apply a low watermarked resolution, "
                        "not subject to the fair use policy")
    elif is_selection:
...
def _draw_new_asset_icon(cTB, layout_row: bpy.types.UILayout, days: int) -> None:
    ...
    tooltip = "Recently Released"
    if days is not None:
        days_str = _t("{days} days ago").format(days=days) if days > 0 else "Today"
        tooltip = _t("Released {days_str}").format(days_str=days_str)
    op.tooltip = tooltip
...
def determine_hdri_sizes(asset_data: AssetData,
    ...
    list_sizes_jpg = []
    list_sizes_light = []
    mapping_size_to_label = {}  # Remember which size corresponds to which format
    for _size in sizes_local:
        tex_maps = asset_type_data.get_maps(size=_size)

        added_hdr_or_exr = False
        for _map in tex_maps:
            format = _map.file_format.lower()
            if format == ".jpg":
                list_sizes_jpg.append(_size)
            elif (not added_hdr_or_exr) and format in [".exr", ".hdr"]:
                list_sizes_light.append(_size)
                mapping_size_to_label[_size] = format[1:].upper()
                added_hdr_or_exr = True  # Make sure we don't add BOTH exr and hdr on same size

    # Determine Light tex size
    if len(list_sizes_light):
        size_light = find_closest_size(size_light_default, list_sizes_light)
    else:
    ...
    # And determine Background tex size
    size_bg = size_bg_default

    if use_jpg_bg and len(list_sizes_jpg):
        size_bg = find_closest_size(size_bg_default, list_sizes_jpg)
        size_bg = f"{size_bg}_JPG"

    elif len(list_sizes_light) > 0:
        size_bg = find_closest_size(size_bg_default, list_sizes_light)
        label = mapping_size_to_label[size_bg]
        size_bg = f"{size_bg}_{label}"
...
def _build_tab_title(cTB, dummy_assets_grid: bool = False) -> None:
    """ Row with Assets (number) to the left,
    and Filter, Refresh and Display options to the right"""
    row = cTB.vBase.row()
    area = cTB.settings["area"]
    ...
    row_right.operator("poliigon.asset_filter",
                       text=curr_filter.map_to_short_value() if set_pressed else _t("Filter"),
                       icon="FILTER",
                       depress=set_pressed)
    # Close button
    if set_pressed:
        reset_filters = row_right.operator("poliigon.poliigon_setting",
                                           icon="PANEL_CLOSE",
                                           depress=set_pressed)
        reset_filters.mode = "close_filters"

    # Refresh and Display options button
    row_right.separator()
...
def _create_icon_asset_layout(
    ...
    if asset_data.is_purchased and asset_data.is_local:
        ...
        days_ago = asset_data.get_days_since_published()
    ...
    if len(sorted_assets) == 0:
        ...
        for _asset_id in sorted_assets:
            ...
            if asset_name == "dummy":
                ...
                if is_tex and not is_downloaded:
