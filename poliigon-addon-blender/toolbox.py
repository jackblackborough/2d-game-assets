...
class c_Toolbox(PoliigonAddon):
    ...
    filters_applied_once: bool = False

    def __init__(self,
                 addon_version: str,
                 api_service: PoliigonConnector = None):
    ...
    def callback_logout_done(self, job: ApiJob) -> None:
        ...
        self.fetching_user_data = True
        self.filters_applied_once = False
    ...
    def _check_default_filter(self, job) -> None:
        """Checks the default filter to be applied.
        The default value can be Recently Added if some asset was viewed on
        the web recently, for this reason if the job is not
        KEY_TAB_RECENTLY_VIEWED, we ignore this check. """

        if job.params.tab != KEY_TAB_RECENTLY_VIEWED:
            return
        if job.params.is_recent_viewed_top_level() and not self.filters_applied_once:
            curr_filter = FilterOptions.get_default_filter(self.user, self.addon_last_view)
            self.set_area(curr_filter.map_to_query())

    def set_area(self, area: str, update_assets: bool = False) -> int:
        with self.lock_thumbs:
            self.thumbs.clear()
        ...
        if self.filters_applied_once:
            self.set_addon_last_view_time()
        self.filters_applied_once = True
        return update
    ...
    def callback_get_asset_done(self, job: ApiJob) -> None:
        ...
        # Check the job to define if we should change the current asset filter
        self._check_default_filter(job)
        ...
        user_assets_tab = params.tab in [KEY_TAB_MY_ASSETS,
                                         KEY_TAB_RECENT_DOWNLOADS,
                                         KEY_TAB_RECENTLY_VIEWED]
    ...
    @staticmethod
    def is_online_top_level_page(asset_filter: str,
                                 search: str,
                                 categories: List[str]
                                 ) -> bool:
        is_online_filter = asset_filter == FilterOptions.ALL_ASSETS.map_to_query()
        no_search = search == ""
        top_level_categories = categories == [CATEGORY_ALL]
        return is_online_filter and no_search and top_level_categories
    ...
    def f_GetAssets(
        ...
        if not self.is_online_top_level_page(area, search, categories):
            self.filters_applied_once = True
